...
import org.scalafmt.config.{ImportSelectors, NewlineCurlyLambda, ScalafmtConfig}
...
import org.scalafmt.util.TokenOps.lastToken
...
object Constants {
  ...
  val DontSplit = Split(NoSplit, 0)
  val DontSplitSeq = Array(DontSplit)

  val SpaceSplit = Split(Space, 0)
  val SpaceSplitSeq = Array(SpaceSplit)
  val NewLine = Split(Newline, 0)
  val NewlineSeq = Array(NewLine)
}
...
class Router(formatOps: FormatOps) {
  ...
  def getSplits1(formatToken: FormatToken, leftOwner: Tree, rightOwner: Tree, newlines: Int, style: ScalafmtConfig) = Option{
    formatToken match {
      ...
      case FormatToken(start @ Interpolation.Start(), _, _) =>
        ...
        Array(
          ...
          Split(
            modification = NoSplit,
            cost = 0,
            ignoreIf = !isStripMargin,
            indents = Seq(Indent(StateColumn, end, Left), Indent(-1, end, Left)),
            policy = policy
          ),
          Split(NoSplit, 0, ignoreIf = isStripMargin, policy = policy)
        )
      case FormatToken(
      ...
      case FormatToken(open @ LeftBrace(), _, _)
        ...
        Array(
          Split(
            if (style.spaces.inImportCurlyBraces) Space else NoSplit,
            0,
            policy = policy
          ),
          Split.withIndent(
      ...
      case tok @ FormatToken(open @ LeftBrace(), right, between) =>
        ...
        Array(
          Split.withToken(
            spaceMod,
            0,
            ignoreIf = skipSingleLineBlock,
            policy = SingleLineBlock(close),
            token = close,
            killOnFail = true
          ),
          Split.withIdentAndOptionToken(
            modification = Space,
            cost = 0,
            ignoreIf =
              style.newlines.alwaysBeforeCurlyBraceLambdaParams ||
                isSelfAnnotation ||
                !startsLambda,
            indent = Indent(lambdaIndent, close, Right),
            token = lambdaArrow,
            policy = lambdaPolicy
          ),
          Split.withIndent(
            ...
            1,
            indent = Indent(2, close, Right),
            policy = newlineBeforeClosingCurly
          )
        )
      case _ => null
    }
  }
  def getSplits2(formatToken: FormatToken, leftOwner: Tree, rightOwner: Tree, newlines: Int, style: ScalafmtConfig) = Option{
    formatToken match {
      ...
      case FormatToken(arrow @ RightArrow(), right, _)
        ...
        Array(
          ...
          Split.withIndent(
            afterCurlyNewlines,
            1,
            indent = Indent(2, endOfFunction, Left)
          )
        )
      case FormatToken(RightArrow(), right, _) if leftOwner.is[Term.Function] =>
        ...
        Array(
          Split(
            Space,
            0,
            ignoreIf = isSingleLineComment(right),
            policy = SingleLineBlock(endOfFunction)
          ),
          Split(Space, 0, ignoreIf = !hasBlock),
          Split.withIndent(
            Newline,
            1 + nestedApplies(leftOwner),
            ignoreIf = hasBlock,
            indent = Indent(indent, endOfFunction, expiresOn)
          )
        )
      // Case arrow
      ...
      case tok @ FormatToken(left, right, between) if startsStatement(tok) =>
        ...
        if (isAnnotation && style.optIn.annotationNewlines)
          Array(Split(newlines2Modification(formatToken), 0))
        else {
          val spaceCouldBeOk =
            newlines == 0 &&
              !left.is[Comment] &&
  ...
  def getSplits3(formatToken: FormatToken, leftOwner: Tree, rightOwner: Tree, newlines: Int, style: ScalafmtConfig) = Option{
    formatToken match {
  ...
  def getSplits4(formatToken: FormatToken, leftOwner: Tree, rightOwner: Tree, newlines: Int, style: ScalafmtConfig) = Option{
    formatToken match {
      ...
      case tok @ FormatToken(LeftParen() | LeftBracket(), right, between)
        ...
        Array(
          Split.withIdentAndToken(
            noSplitModification,
            0,
            policy = noSplitPolicy,
            token = expirationToken,
            indent = Indent(noSplitIndent, close, Right)
          ),
          Split.withIdentAndToken(
            newlineModification,
            (1 + nestedPenalty + lhsPenalty) * bracketMultiplier,
            token = expirationToken,
            indent = Indent(indent, close, Right),
          ...
          Split.withIdentAndToken(
            ...
            ignoreIf =
              singleArgument || tooManyArguments ||
                skipOpenParenAlign,
            indent = Indent(StateColumn, close, Right),
            token = expirationToken
          ),
          Split.withIdentAndToken(
            Newline,
  ...
  def getSplits5(formatToken: FormatToken, leftOwner: Tree, rightOwner: Tree, newlines: Int, style: ScalafmtConfig) = Option{
    formatToken match {
      ...
      case FormatToken(left, Colon(), _)
        ...
        Array(
          Split(sameLineSplit, 0, policy = penalizeNewlines),
          // Spark style guide allows this:
          // https://github.com/databricks/scala-style-guide#indent
          Split.withIndent(
            Newline,
            Constants.SparkColonNewline,
            policy = penalizeNewlines,
            indent = Indent(style.continuationIndent.defnSite, expire, Left)
          )
        )
      case FormatToken(Colon(), _, _)
        ...
        Array(
          Split(
            Space,
            0,
            policy = SingleLineBlock(expire, disallowSingleLineComments = false)
          )
        )
            ...
            val optToken = nextComma.map(
              _ =>
                OptimalToken(
                  ...
                  killOnFail = true))
            Array(
              Split(Space, 0, optimalAt = optToken, policy = singleLine),
              Split(Newline, 1, optimalAt = optToken, policy = singleLine),
              // next argument doesn't fit on a single line, break on comma before
              // and comma after.
              Split(Newline, 2, optimalAt = optToken, policy = breakOnNextComma)
            )
          case _ if isInfix =>
            Array(
          ...
          case _ =>
            ...
            Array(
              ...
              Split.withIndent(
                Newline,
                1,
                ignoreIf = rhsIsAttachedComment,
                indent = Indent(Num(indent), right, ExpiresOn.Right)
              )
            )
        }
  ...
  def getSplits6(formatToken: FormatToken, leftOwner: Tree, rightOwner: Tree, newlines: Int, style: ScalafmtConfig) = Option{
    formatToken match {
          ...
          case _ =>
            ...
            Array(
              Split.withIdentAndToken(
                ...
                token = expire,
                killOnFail = false,
                indent = Indent(spaceIndent, expire, Left)
              ),
              Split.withIndent(
                mod,
                1 + penalty,
                ignoreIf = jsNative,
                indent = Indent(2, expire, Left)
              )
            )
        }
      case tok @ FormatToken(left, dot @ Dot() `:chain:` chain, _)
        ...
        if (TokenOps.isSymbolicIdent(left))
          ...
          Array(
            Split(NoSplit, 0, ignoreIf = ignoreNoSplit, policy = noSplitPolicy),
            Split.withIndent(
              Newline.copy(acceptNoSplit = true),
              2 + nestedPenalty + chainLengthPenalty,
              policy = newlinePolicy,
              indent = Indent(2, optimalToken, Left)
            )
          )
      // ApplyUnary
      ...
      case FormatToken(At(), right @ Ident(_), _) =>
        ...
        Array(Split(identModification(right), 0))
        ...
        rightOwner match {
          ...
          case t @ WithChain(top) =>
            ...
            if (isFirstWith) {
              ...
              Array(SpaceSplit, Split(Newline, 1))
            }
          case _ =>
            SpaceSplitSeq
        }
  ...
  def getSplits7(formatToken: FormatToken, leftOwner: Tree, rightOwner: Tree, newlines: Int, style: ScalafmtConfig) = Option{
    formatToken match {
      ...
      case FormatToken(KwIf(), _, _) if leftOwner.is[Term.If] =>
        ...
        Array(
          ...
          Split(Space, 1, policy = breakOnlyBeforeElse)
        )
      case FormatToken(close @ RightParen(), right, between)
        ...
        Array(
          Split(
            Space,
            0,
            ignoreIf = attachedComment || newlines > 0,
      ...
      case FormatToken(RightBrace(), KwElse(), _) =>
        if (style.newlines.alwaysBeforeElseAfterCurlyIf) NewlineSeq
        else SpaceSplitSeq
      ...
      case tok @ FormatToken(els @ KwElse(), _, _)
        ...
        Array(
          ...
          Split.withIndent(Newline, 1, indent = Indent(2, expire, Left))
        )
      ...
      case FormatToken(open @ LeftParen(), right, _) =>
        ...
        Array(
          Split.withIndent(
            ...
            ignoreIf = !isConfig,
            policy = breakOnClose,
            indent = Indent(style.continuationIndent.callSite, close, Right)
          ),
          Split.withIndent(
            NoSplit,
            0,
            ignoreIf = isConfig,
            indent = Indent(indent, close, Left),
            policy = penalizeAllNewlines(close, 1)
          )
        )
      case _ => null
    }
  }
  def getSplits8(formatToken: FormatToken, leftOwner: Tree, rightOwner: Tree, newlines: Int, style: ScalafmtConfig) = Option{
    formatToken match {
      ...
      case tok @ FormatToken(op @ Ident(_), right, between)
        ...
        Array(infixSplit(leftOwner, op, args, formatToken))
      case FormatToken(left, op @ Ident(_), between)
        ...
        Array(infixSplit(rightOwner, op, args, formatToken))
      case opt
        if style.optIn.annotationNewlines &&
          optionalNewlines(hash(opt.right)) =>
        Array(Split(newlines2Modification(opt), 0))
      // Pat
        ...
        Array(
          ...
          Split
            .withIndent(
              ...
              indent = Indent(2, expire, Left)
            ) // case body indented by 2. TODO if found multiple times specialize constructor for multiple indents
      ...
      case tok @ FormatToken(_, cond @ KwIf(), _) if rightOwner.is[Case] =>
        ...
        Array(
          ...
          Split(Newline, 1, policy = penalizeNewlineByNesting(cond, arrow))
        )
      // Inline comment
      case FormatToken(_, c: Comment, between) =>
        Array(Split(newlines2Modification(between), 0))
      ...
      case tok @ FormatToken(arrow @ LeftArrow(), _, _)
        ...
        Array(
          // Either everything fits in one line or break on =>
          Split.withIndent(Space, 0, indent = Indent(indent, lastToken, Left))
        )
      case tok @ FormatToken(KwYield(), right, _)
        ...
        Array(
          // Either everything fits in one line or break on =>
          Split(Space, 0, policy = SingleLineBlock(lastToken)),
          Split.withIndent(Newline, 1, indent = Indent(2, lastToken, Left))
        )
      // Interpolation
      ...
      case FormatToken(_, RightParen(), _) =>
        ...
        Array(
          Split(mod, 0) //cannot get this from constants for some reason
        )
  ...
  private def getSplits(formatToken: FormatToken): Array[Split] = {
    ...
    getSplits1(formatToken,leftOwner,rightOwner,newlines,style)
    .orElse(getSplits2(formatToken,leftOwner,rightOwner,newlines,style))
    .orElse(getSplits3(formatToken,leftOwner,rightOwner,newlines,style))
    .orElse(getSplits4(formatToken,leftOwner,rightOwner,newlines,style))
    .orElse(getSplits5(formatToken,leftOwner,rightOwner,newlines,style))
    .orElse(getSplits6(formatToken,leftOwner,rightOwner,newlines,style))
    .orElse(getSplits7(formatToken,leftOwner,rightOwner,newlines,style))
    .orElse(getSplits8(formatToken,leftOwner,rightOwner,newlines,style))
        .getOrElse{
          formatToken match {
            case tok =>
              logger.debug("MISSING CASE:\n" + log(tok))
              Array.empty[Split] // No solution available, partially format tree.
          }
        }
  }

  // TODO(olafur) replace cache with array of seq[split]
  private val cache = mutable.Map.empty[FormatToken, Array[Split]]
  ...
  /**
    ...
    */
  def getSplitsMemo(formatToken: FormatToken): Array[Split] =
      ...
      formatToken, {
        ...
        formatToken match {
          ...
          case FormatToken(c @ Comment(_), _, _) if c.syntax.startsWith("//") =>
            ...
            if (newlineSplits.isEmpty) NewlineSeq
            else newlineSplits
          case FormatToken(_, c: Comment, between)
